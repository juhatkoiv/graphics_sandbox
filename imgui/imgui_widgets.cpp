...
static void stb_textedit_replace(ImGuiInputTextState* str, STB_TexteditState* inputState, const STB_TEXTEDIT_CHARTYPE* text, int text_len)
{
    stb_text_makeundo_replace(str, inputState, 0, str->CurLenW, text_len);
    ImStb::STB_TEXTEDIT_DELETECHARS(str, 0, str->CurLenW);
    if (text_len <= 0)
        return;
    if (ImStb::STB_TEXTEDIT_INSERTCHARS(str, 0, text, text_len))
    {
        inputState->cursor = text_len;
        inputState->has_preferred_x = 0;
        return;
    }
...
static void InputTextReconcileUndoStateAfterUserCallback(ImGuiInputTextState* inputState, const char* new_buf_a, int new_length_a)
{
    ImGuiContext& g = *GImGui;
    const ImWchar* old_buf = inputState->TextW.Data;
    const int old_length = inputState->CurLenW;
    ...
    if (insert_len > 0 || delete_len > 0)
        if (STB_TEXTEDIT_CHARTYPE* p = stb_text_createundo(&inputState->Stb.undostate, first_diff, delete_len, insert_len))
            for (int i = 0; i < delete_len; i++)
                p[i] = ImStb::STB_TEXTEDIT_GETCHAR(inputState, first_diff + i);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImGuiInputTextState* inputState = GetInputTextState(id);
    ...
    const bool user_scroll_finish = is_multiline && inputState != NULL && g.ActiveId == 0 && g.ActiveIdPreviousFrame == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    const bool user_scroll_active = is_multiline && inputState != NULL && g.ActiveId == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    bool clear_active_id = false;
    bool select_all = false;

    float scroll_y = is_multiline ? draw_window->Scroll.y : FLT_MAX;

    const bool init_changed_specs = (inputState != NULL && inputState->Stb.single_line != !is_multiline);
    const bool init_make_active = (user_clicked || user_scroll_finish || input_requested_by_nav || input_requested_by_tabbing);
    const bool init_state = (init_make_active || user_scroll_active);
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        // Access state even if we don't own it yet.
        inputState = &g.InputTextState;
        inputState->CursorAnimReset();

        // Take a copy of the initial buffer value (both in original UTF-8 format and converted to wchar)
        // From the moment we focused we are ignoring the content of 'buf' (unless we are in read-only mode)
        const int buf_len = (int)strlen(buf);
        inputState->InitialTextA.resize(buf_len + 1);    // UTF-8. we use +1 to make sure that .Data is always pointing to at least an empty string.
        memcpy(inputState->InitialTextA.Data, buf, buf_len + 1);

        // Preserve cursor position and undo/redo stack if we come back to same widget
        // FIXME: Since we reworked this on 2022/06, may want to differenciate recycle_cursor vs recycle_undostate?
        bool recycle_state = (inputState->ID == id && !init_changed_specs);
        if (recycle_state && (inputState->CurLenA != buf_len || (inputState->TextAIsValid && strncmp(inputState->TextA.Data, buf, buf_len) != 0)))
            recycle_state = false;

        // Start edition
        const char* buf_end = NULL;
        inputState->ID = id;
        inputState->TextW.resize(buf_size + 1);          // wchar count <= UTF-8 count. we use +1 to make sure that .Data is always pointing to at least an empty string.
        inputState->TextA.resize(0);
        inputState->TextAIsValid = false;                // TextA is not valid yet (we will display buf until then)
        inputState->CurLenW = ImTextStrFromUtf8(inputState->TextW.Data, buf_size, buf, NULL, &buf_end);
        inputState->CurLenA = (int)(buf_end - buf);      // We can't get the result from ImStrncpy() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.

        if (recycle_state)
        {
            // Recycle existing cursor/selection/undo stack but clamp position
            // Note a single mouse click will override the cursor/position immediately by calling stb_textedit_click handler.
            inputState->CursorClamp();
        }
        else
        {
            inputState->ScrollX = 0.0f;
            stb_textedit_initialize_state(&inputState->Stb, !is_multiline);
        }
        ...
        if (flags & ImGuiInputTextFlags_AlwaysOverwrite)
            inputState->Stb.insert_mode = 1; // stb field name is indeed incorrect (see #2863)
    }

    if (g.ActiveId != id && init_make_active)
    {
        IM_ASSERT(inputState && inputState->ID == id);
        SetActiveID(id, window);
        SetFocusID(id, window);
        FocusWindow(window);
    ...
    if (g.ActiveId == id && inputState == NULL)
        ClearActiveID();
    ...
    const bool render_cursor = (g.ActiveId == id) || (inputState && user_scroll_active);
    bool render_selection = inputState && (inputState->HasSelection() || select_all) && (RENDER_SELECTION_WHEN_INACTIVE || render_cursor);
    bool value_changed = false;
    bool enter_pressed = false;

    // When read-only we always use the live data passed to the function
    // FIXME-OPT: Because our selection/cursor code currently needs the wide text we need to convert it when active, which is not ideal :(
    if (is_readonly && inputState != NULL && (render_cursor || render_selection))
    {
        const char* buf_end = NULL;
        inputState->TextW.resize(buf_size + 1);
        inputState->CurLenW = ImTextStrFromUtf8(inputState->TextW.Data, inputState->TextW.Size, buf, NULL, &buf_end);
        inputState->CurLenA = (int)(buf_end - buf);
        inputState->CursorClamp();
        render_selection &= inputState->HasSelection();
    }

    // Select the buffer to render.
    const bool buf_display_from_state = (render_cursor || render_selection || g.ActiveId == id) && !is_readonly && inputState && inputState->TextAIsValid;
    const bool is_displaying_hint = (hint != NULL && (buf_display_from_state ? inputState->TextA.Data : buf)[0] == 0);
    ...
    if (g.ActiveId == id)
    {
        IM_ASSERT(inputState != NULL);
        backup_current_text_length = inputState->CurLenA;
        inputState->Edited = false;
        inputState->BufCapacityA = buf_size;
        inputState->Flags = flags;
        ...
        // Edit in progress
        const float mouse_x = (io.MousePos.x - frame_bb.Min.x - style.FramePadding.x) + inputState->ScrollX;
        ...
        if (select_all)
        {
            ...
            inputState->SelectedAllMouseLock = true;
        }
        else if (hovered && io.MouseClickedCount[0] >= 2 && !io.KeyShift)
        {
            stb_textedit_click(inputState, &inputState->Stb, mouse_x, mouse_y);
            ...
            if ((multiclick_count % 2) == 0)
            {
                ...
                const bool is_bol = (inputState->Stb.cursor == 0) || ImStb::STB_TEXTEDIT_GETCHAR(inputState, inputState->Stb.cursor - 1) == '\n';
                if (STB_TEXT_HAS_SELECTION(&inputState->Stb) || !is_bol)
                    inputState->OnKeyPressed(STB_TEXTEDIT_K_WORDLEFT);
                //state->OnKeyPressed(STB_TEXTEDIT_K_WORDRIGHT | STB_TEXTEDIT_K_SHIFT);
                if (!STB_TEXT_HAS_SELECTION(&inputState->Stb))
                    ImStb::stb_textedit_prep_selection_at_cursor(&inputState->Stb);
                inputState->Stb.cursor = ImStb::STB_TEXTEDIT_MOVEWORDRIGHT_MAC(inputState, inputState->Stb.cursor);
                inputState->Stb.select_end = inputState->Stb.cursor;
                ImStb::stb_textedit_clamp(inputState, &inputState->Stb);
            }
            else
            {
                // Triple-click: Select line
                const bool is_eol = ImStb::STB_TEXTEDIT_GETCHAR(inputState, inputState->Stb.cursor) == '\n';
                inputState->OnKeyPressed(STB_TEXTEDIT_K_LINESTART);
                inputState->OnKeyPressed(STB_TEXTEDIT_K_LINEEND | STB_TEXTEDIT_K_SHIFT);
                inputState->OnKeyPressed(STB_TEXTEDIT_K_RIGHT | STB_TEXTEDIT_K_SHIFT);
                if (!is_eol && is_multiline)
                {
                    ImSwap(inputState->Stb.select_start, inputState->Stb.select_end);
                    inputState->Stb.cursor = inputState->Stb.select_end;
                }
                inputState->CursorFollow = false;
            }
            inputState->CursorAnimReset();
        }
        else if (io.MouseClicked[0] && !inputState->SelectedAllMouseLock)
        {
        ...
        else if (io.MouseDown[0] && !inputState->SelectedAllMouseLock && (io.MouseDelta.x != 0.0f || io.MouseDelta.y != 0.0f))
        {
            stb_textedit_drag(inputState, &inputState->Stb, mouse_x, mouse_y);
            inputState->CursorAnimReset();
            inputState->CursorFollow = true;
        }
        if (inputState->SelectedAllMouseLock && !io.MouseDown[0])
            inputState->SelectedAllMouseLock = false;
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        inputState->Stb.row_count_per_page = row_count_per_page;
        ...
        const bool is_cut   = ((is_shortcut_key && IsKeyPressed(ImGuiKey_X)) || (is_shift_key_only && IsKeyPressed(ImGuiKey_Delete))) && !is_readonly && !is_password && (!is_multiline || inputState->HasSelection());
        const bool is_copy  = ((is_shortcut_key && IsKeyPressed(ImGuiKey_C)) || (is_ctrl_key_only  && IsKeyPressed(ImGuiKey_Insert))) && !is_password && (!is_multiline || inputState->HasSelection());
        ...
        if (IsKeyPressed(ImGuiKey_LeftArrow))                        { inputState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINESTART : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT : STB_TEXTEDIT_K_LEFT) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_RightArrow))                  { inputState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINEEND : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDRIGHT : STB_TEXTEDIT_K_RIGHT) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - g.FontSize, 0.0f)); else inputState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline)   { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.FontSize, GetScrollMaxY())); else inputState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) && is_multiline)      { inputState->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_PageDown) && is_multiline)    { inputState->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_Home))                        { inputState->OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTSTART | k_mask : STB_TEXTEDIT_K_LINESTART | k_mask); }
        else if (IsKeyPressed(ImGuiKey_End))                         { inputState->OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTEND | k_mask : STB_TEXTEDIT_K_LINEEND | k_mask); }
        else if (IsKeyPressed(ImGuiKey_Delete) && !is_readonly && !is_cut) { inputState->OnKeyPressed(STB_TEXTEDIT_K_DELETE | k_mask); }
        else if (IsKeyPressed(ImGuiKey_Backspace) && !is_readonly)
        {
            if (!inputState->HasSelection())
            {
                if (is_wordmove_key_down)
                    inputState->OnKeyPressed(STB_TEXTEDIT_K_WORDLEFT | STB_TEXTEDIT_K_SHIFT);
                else if (is_osx && io.KeySuper && !io.KeyAlt && !io.KeyCtrl)
                    inputState->OnKeyPressed(STB_TEXTEDIT_K_LINESTART | STB_TEXTEDIT_K_SHIFT);
            }
            inputState->OnKeyPressed(STB_TEXTEDIT_K_BACKSPACE | k_mask);
        }
        ...
        {
            inputState->OnKeyPressed(is_undo ? STB_TEXTEDIT_K_UNDO : STB_TEXTEDIT_K_REDO);
        ...
        else if (is_cut || is_copy)
        {
            ...
            {
                const int ib = inputState->HasSelection() ? ImMin(inputState->Stb.select_start, inputState->Stb.select_end) : 0;
                const int ie = inputState->HasSelection() ? ImMax(inputState->Stb.select_start, inputState->Stb.select_end) : inputState->CurLenW;
                const int clipboard_data_len = ImTextCountUtf8BytesFromStr(inputState->TextW.Data + ib, inputState->TextW.Data + ie) + 1;
                char* clipboard_data = (char*)IM_ALLOC(clipboard_data_len * sizeof(char));
                ImTextStrToUtf8(clipboard_data, clipboard_data_len, inputState->TextW.Data + ib, inputState->TextW.Data + ie);
            ...
            if (is_cut)
            {
                if (!inputState->HasSelection())
                    inputState->SelectAll();
                inputState->CursorFollow = true;
                stb_textedit_cut(inputState, &inputState->Stb);
            }
        }
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                {
                    stb_textedit_paste(inputState, &inputState->Stb, clipboard_filtered, clipboard_filtered_len);
                    inputState->CursorFollow = true;
                }
        ...
        render_selection |= inputState->HasSelection() && (RENDER_SELECTION_WHEN_INACTIVE || render_cursor);
    }
    ...
    if (g.ActiveId == id)
    {
        ...
        if (cancel_edit)
        {
            ...
            if (!is_readonly && strcmp(buf, inputState->InitialTextA.Data) != 0)
            {
                // Push records into the undo stack so we can CTRL+Z the revert operation itself
                apply_new_text = inputState->InitialTextA.Data;
                apply_new_text_length = inputState->InitialTextA.Size - 1;
                ...
                stb_textedit_replace(inputState, &inputState->Stb, w_text.Data, (apply_new_text_length > 0) ? (w_text.Size - 1) : 0);
            }
        }
        ...
        if (!is_readonly)
        {
            inputState->TextAIsValid = true;
            inputState->TextA.resize(inputState->TextW.Size * 4 + 1);
            ImTextStrToUtf8(inputState->TextA.Data, inputState->TextA.Size, inputState->TextW.Data, NULL);
        }
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                else if ((flags & ImGuiInputTextFlags_CallbackEdit) && inputState->Edited)
                {
                ...
                if (event_flag)
                {
                    ...
                    char* callback_buf = is_readonly ? buf : inputState->TextA.Data;
                    callback_data.EventKey = event_key;
                    callback_data.Buf = callback_buf;
                    callback_data.BufTextLen = inputState->CurLenA;
                    callback_data.BufSize = inputState->BufCapacityA;
                    callback_data.BufDirty = false;

                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar buffer, see https://github.com/nothings/stb/issues/188)
                    ImWchar* text = inputState->TextW.Data;
                    const int utf8_cursor_pos = callback_data.CursorPos = ImTextCountUtf8BytesFromStr(text, text + inputState->Stb.cursor);
                    const int utf8_selection_start = callback_data.SelectionStart = ImTextCountUtf8BytesFromStr(text, text + inputState->Stb.select_start);
                    const int utf8_selection_end = callback_data.SelectionEnd = ImTextCountUtf8BytesFromStr(text, text + inputState->Stb.select_end);

                    // Call user code
                    callback(&callback_data);

                    // Read back what user may have modified
                    callback_buf = is_readonly ? buf : inputState->TextA.Data; // Pointer may have been invalidated by a resize callback
                    IM_ASSERT(callback_data.Buf == callback_buf);         // Invalid to modify those fields
                    IM_ASSERT(callback_data.BufSize == inputState->BufCapacityA);
                    IM_ASSERT(callback_data.Flags == flags);
                    const bool buf_dirty = callback_data.BufDirty;
                    if (callback_data.CursorPos != utf8_cursor_pos || buf_dirty)            { inputState->Stb.cursor = ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.CursorPos); inputState->CursorFollow = true; }
                    if (callback_data.SelectionStart != utf8_selection_start || buf_dirty)  { inputState->Stb.select_start = (callback_data.SelectionStart == callback_data.CursorPos) ? inputState->Stb.cursor : ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionStart); }
                    if (callback_data.SelectionEnd != utf8_selection_end || buf_dirty)      { inputState->Stb.select_end = (callback_data.SelectionEnd == callback_data.SelectionStart) ? inputState->Stb.select_start : ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionEnd); }
                    if (buf_dirty)
                    {
                        IM_ASSERT(callback_data.BufTextLen == (int)strlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
                        InputTextReconcileUndoStateAfterUserCallback(inputState, callback_data.Buf, callback_data.BufTextLen); // FIXME: Move the rest of this block inside function and rename to InputTextReconcileStateAfterUserCallback() ?
                        if (callback_data.BufTextLen > backup_current_text_length && is_resizable)
                            inputState->TextW.resize(inputState->TextW.Size + (callback_data.BufTextLen - backup_current_text_length)); // Worse case scenario resize
                        inputState->CurLenW = ImTextStrFromUtf8(inputState->TextW.Data, inputState->TextW.Size, callback_data.Buf, NULL);
                        inputState->CurLenA = callback_data.BufTextLen;  // Assume correct length and valid UTF-8 from user, saves us an extra strlen()
                        inputState->CursorAnimReset();
                    }
                }
            }
            ...
            if (!is_readonly && strcmp(inputState->TextA.Data, buf) != 0)
            {
                apply_new_text = inputState->TextA.Data;
                apply_new_text_length = inputState->CurLenA;
            }
        }

        // Clear temporary user storage
        inputState->Flags = ImGuiInputTextFlags_None;
    }
    ...
    const char* buf_display = buf_display_from_state ? inputState->TextA.Data : buf; //-V595
    ...
    if (render_cursor || render_selection)
    {
        IM_ASSERT(inputState != NULL);
        if (!is_displaying_hint)
            buf_display_end = buf_display + inputState->CurLenA;
        ...
        const ImWchar* text_begin = inputState->TextW.Data;
...

        {
            ...
            if (render_cursor)
            {
                searches_input_ptr[0] = text_begin + inputState->Stb.cursor;
                searches_result_line_no[0] = -1;
                searches_remaining++;
            }
            if (render_selection)
            {
                searches_input_ptr[1] = text_begin + ImMin(inputState->Stb.select_start, inputState->Stb.select_end);
        ...
        // Scroll
        if (render_cursor && inputState->CursorFollow)
        {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if (cursor_offset.x < inputState->ScrollX)
                    inputState->ScrollX = IM_FLOOR(ImMax(0.0f, cursor_offset.x - scroll_increment_x));
                else if (cursor_offset.x - visible_width >= inputState->ScrollX)
                    inputState->ScrollX = IM_FLOOR(cursor_offset.x - visible_width + scroll_increment_x);
            }
            else
            {
                inputState->ScrollX = 0.0f;
        ...
        // Draw selection
        const ImVec2 draw_scroll = ImVec2(inputState->ScrollX, 0.0f);
        if (render_selection)
        {
            const ImWchar* text_selected_begin = text_begin + ImMin(inputState->Stb.select_start, inputState->Stb.select_end);
            const ImWchar* text_selected_end = text_begin + ImMax(inputState->Stb.select_start, inputState->Stb.select_end);
        ...
        if (render_cursor)
        {
            inputState->CursorAnim += io.DeltaTime;
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (inputState->CursorAnim <= 0.0f) || ImFmod(inputState->CursorAnim, 1.20f) <= 0.80f;
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* inputState)
{
#ifndef IMGUI_DISABLE_METRICS_WINDOW
    ImGuiContext& g = *GImGui;
    ImStb::STB_TexteditState* stb_state = &inputState->Stb;
    ImStb::StbUndoState* undo_state = &stb_state->undostate;
    Text("ID: 0x%08X, ActiveID: 0x%08X", inputState->ID, g.ActiveId);
    Text("CurLenW: %d, CurLenA: %d, Cursor: %d, Selection: %d..%d", inputState->CurLenA, inputState->CurLenW, stb_state->cursor, stb_state->select_start, stb_state->select_end);
